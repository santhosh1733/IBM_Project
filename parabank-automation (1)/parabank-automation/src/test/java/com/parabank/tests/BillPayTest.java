...
public class BillPayTest extends BaseTest {
    ...
    public void loadBillPayTestData() {
        List<Map<String, String>> rows = ExcelUtils.getSheetData(
                ConfigReader.get("testdata.path"), "BillPay");
    ...
    @Test(groups = {"smoke", "e2e"},
            description = "PB_AUTO_01 - Verify end-to-end successful Bill Payment")
    public void pbAuto01_verifyEndToEndSuccessfulBillPayment() {
        ...
        info("Step 2: Navigate to Bill Pay and enter valid payee/payment details. "
                + "Payee=" + data.get("PayeeName") + ", Amount=" + data.get("Amount")
        ...
        Assert.assertTrue(billPay.isPaymentSuccessful(),
                "Bill Payment Complete confirmation was not displayed.");
        ...
        Assert.assertEquals(billPay.getConfirmedFromAccount(), sourceAccount,
                "Confirmed source account is incorrect.");
        pass("Verification Passed: Payee=" + data.get("PayeeName")
                + ", Amount=" + formatCurrency(data.get("Amount"))
                + " and From Account=" + sourceAccount + " are correct.");
    }

    @Test(groups = {"functional", "negative"},
            description = "PB_AUTO_02 - Verify Account and Verify Account mismatch validation")
    public void pbAuto02_verifyAccountNumberMismatchValidation() {
        ...
        info("Step 2: Navigate to Bill Pay and enter valid payee details with mismatching "
                + "Account and Verify Account values. Account=" + data.get("PayeeAccount")
                + ", Verify Account=" + data.get("VerifyAccount") + ".");
        BillPayPage billPay = fillCompleteBillPayForm(overview, data, sourceAccount, true);
        ...
        Assert.assertEquals(billPay.getAccountMismatchErrorText(),
                "The account numbers do not match.",
                "Unexpected account-number mismatch message.");
        pass("Verification Passed: Validation message is 'The account numbers do not match.'.");
    }

    @Test(groups = {"integration"},
            description = "PB_AUTO_03 - Verify account balance after successful Bill Payment")
    public void pbAuto03_verifyAccountBalanceAfterSuccessfulBillPayment() {
        ...
        info("Step 3: Navigate to Bill Pay, enter valid data and submit payment of "
        ...
        pass("Verification Passed: Balance changed from " + formatCurrency(balanceBefore)
                + " to " + formatCurrency(balanceAfter)
                + ", exactly matching payment amount " + formatCurrency(paymentAmount) + ".");
    }

    @Test(groups = {"integration", "e2e"},
            description = "PB_AUTO_04 - Verify successful Bill Payment in transaction history")
    public void pbAuto04_verifyBillPaymentTransactionHistory() {
        ...
        String sourceAccount = overview.getFirstAccountId();
        info("Step 2: Navigate to Bill Pay and submit a valid payment. Payee="
                + data.get("PayeeName") + ", Amount=" + data.get("Amount")
                + ", From Account=" + sourceAccount + ".");
        BillPayPage billPay = fillCompleteBillPayForm(overview, data, sourceAccount, true)
                .submitPayment();
        assertNoBackendError(billPay);
        ...
        Assert.assertTrue(details.isTransactionTableDisplayed(),
                "Transaction-history table was not displayed.");
        ...
        Assert.assertTrue(details.hasBillPaymentTransaction(
                        data.get("PayeeName"), data.get("Amount")),
                "New Bill Pay transaction was not found with the expected payee and debit amount.");
        pass("Verification Passed: Transaction history contains Payee=" + data.get("PayeeName")
                + " and Amount=" + formatCurrency(data.get("Amount")) + ".");
    }

    @Test(groups = {"functional", "negative"},
            description = "PB_AUTO_05 - Verify Amount mandatory validation")
    public void pbAuto05_verifyPaymentAmountMandatoryValidation() {
        ...
        String sourceAccount = overview.getFirstAccountId();
        info("Step 2: Navigate to Bill Pay and enter valid payee/account details while leaving "
                + "Amount blank.");
        BillPayPage billPay = fillCompleteBillPayForm(overview, data, sourceAccount, false)
                .leaveAmountBlank();
        ...
        Assert.assertTrue(billPay.isAmountRequiredErrorDisplayed(),
                "Amount-required validation was not displayed.");
    ...
    private BillPayPage fillCompleteBillPayForm(BillPayAccountOverviewPage overview,
    ...
    private Map<String, String> data(String testCaseId) {
        ...
        if (row == null) {
            throw new IllegalStateException(
                    "BillPay test data row not found for TestCaseId: " + testCaseId);
        }
        return row;
    }
    ...
    private void assertNoBackendError(BillPayPage billPay) {
        if (billPay.isBillPayErrorDisplayed()) {
            Assert.fail("ParaBank Bill Pay backend returned an application error: "
                    + billPay.getBillPayErrorText());
        }
    }
